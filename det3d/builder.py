...
def build_dbsampler(cfg, logger=None):
    ...
    client = getattr(file_client, cfg.client_cfg.name)(
        **cfg.client_cfg.get('kwargs', {})
    )
    db_infos = client.load_pickle(info_path)
    grot_range = list(grot_range)
    if len(grot_range) == 0:
        grot_range = None
    sampler = DataBaseSamplerV2(
        db_infos, client, groups, db_prepor, rate, grot_range, logger=logger
    )
