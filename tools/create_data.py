...
client = getattr(file_client, client_cfg['name'])(
...
def waymo_data_prep(root_path, processed_data_tag, split, nsweeps=1):
    waymo_ds.create_waymo_infos(root_path, client, processed_data_tag, split=split, nsweeps=nsweeps)
    if split == 'train': 
        create_groundtruth_database(
            "WAYMO",
            root_path,
